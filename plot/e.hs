module E where
    ...
    taylorSeries :: Double -> Int -> Double
    taylorSeries x y = sum $ take y $ zipWith (\n facn -> (x ** fromIntegral n) / (fromIntegral facn)) [0..] fac
        where
            fac = scanl (*) 1 [1..] 
    ...
    -- Gera a série de Taylor para e^x
    taylorSeries' :: Double -> Int -> Double
    taylorSeries' x y = sum $ take y $ scanl (\acc k -> acc * x / k) 1 [1..]